<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AssetTransfer.aspx.cs" Inherits="AssetManagement.Views.Administrator.AssetTransfer" %>
...
<script runat="server">
    ...
    protected void UpdateData()
    {
        ...
        UpdateAssetTransfer(_objid.ToString());
    }

    protected void UpdateAssetTransfer(string OBJID)
    {
        //Please Edit
        string _refCode = Code.CreateSequenceCode("T" + DateTime.Today.Month.ToString() + DateTime.Today.Year.ToString(), ((int)WorkflowType.Disposal).ToString(), 2, AssetWorkflow.GetAssetTransferMaxNumber().ToString(), 3);

        AssetCatalog.UpdateAssetTransfer(OBJID.ToString(), _refCode, ((int)ApprovalStatus.Unapproved).ToString(), DateTime.Now.ToString(), ddlOrganization.SelectedValue, ddlOrganization.SelectedItem.Text, ddlEmployee.SelectedValue, ddlEmployee.SelectedItem.Text.Split('-')[ddlEmployee.SelectedItem.Text.Split('-').Length - 1].Trim(), ddlLocation.SelectedValue, ddlLocation.SelectedItem.Text, ddlNewOrganization.SelectedValue, ddlNewOrganization.SelectedItem.Text, ddlNewHolder.SelectedValue, ddlNewHolder.SelectedItem.Text.Split('-')[ddlNewHolder.SelectedItem.Text.Split('-').Length - 1].Trim(), ddlNewLocation.SelectedValue, ddlNewLocation.SelectedItem.Text, (string)Session["username"]);
        
        AssetWorkflow.InsertWorkFlow(_refCode, ((int)WorkflowUser.UnitUser).ToString(), ((int)WorkflowType.Transfer).ToString(), Session["username"].ToString(), Session["poscd"].ToString(), txtNote.Text, ((int)WorkflowStatus.Request).ToString(), Session["username"].ToString());
    }
