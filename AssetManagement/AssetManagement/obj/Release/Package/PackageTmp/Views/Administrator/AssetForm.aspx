<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AssetForm.aspx.cs" Inherits="AssetManagement.Views.Administrator.AssetForm" %>
...
<script runat="server">
    ...
    protected void SetEmployee()
    {
        ...
        if (Session["EmployeeSet"] == null)
        {
            data = UserCatalog.GetEmployee();
            ...
            Session["EmployeeSet"] = data;
        }
        else
        {
            data = (DataTable)Session["EmployeeSet"];
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (passcode == Session["password"].ToString())
        {
            ...
            {
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi nama aset tidak boleh kosong" + "');", true);
            }
            else if (txtIdNumber.Text == string.Empty)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi ID Aset tidak boleh kosong" + "');", true);
            }
            else if (txtAssetNumber.Text == string.Empty)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi nomor aset tidak boleh kosong" + "');", true);
            }
            else if (txtNumber.Text == string.Empty)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi nomor inventaris tidak boleh kosong" + "');", true);
            }
            else if (txtNilaiAset.Text == string.Empty)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Nilai perolehan tidak boleh kosong" + "');", true);
            }
            else if (!Validation.IsValidInteger(txtNilaiAset.Text))
            {
                if (!Validation.IsValidInteger(txtNilaiAset.Text.Split(' ')[txtNilaiAset.Text.Split(' ').Length-1].Split(',')[0].Replace(".", string.Empty)))
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Isikan nilai perolehan dengan benar berdasarkan format mata uang rupiah atau integer" + "');", true);
                }
                else if (!Validation.IsPositiveValue((Convert.ToInt64(txtNilaiAset.Text.Split(' ')[txtNilaiAset.Text.Split(' ').Length-1].Split(',')[0].Replace(".", string.Empty)))))
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Isikan nilai perolehan dengan nilai bilangan positif" + "');", true);
                }
                else if (txtBegda.Text == string.Empty)
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi tanggal perolehan tidak boleh kosong" + "');", true);
                }
                else if (!rbtNonFisik.Checked && !rbtStatusBaik.Checked && !rbtStatusRusak.Checked && !rbtTidakProduktif.Checked)
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi status aset tidak boleh kosong" + "');", true);
                }
                else if (txtPeruntukan.Text == string.Empty)
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi peruntukan aset tidak boleh kosong" + "');", true);
                }
                else if (txtMTBF.Text == string.Empty)
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi nilai MTBF tidak boleh kosong" + "');", true);
                }
                else if (!Validation.IsValidInteger(txtMTBF.Text))
                {
                    ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Nilai MTBF harus bilangan bulat positif" + "');", true);
                }
                else
                {
                    UpdateData();
    ...
    protected void UpdateData()
    {
        ...
        string _nilai = txtNilaiAset.Text;
        if (!Validation.IsValidInteger(txtNilaiAset.Text))
        {
            _nilai = txtNilaiAset.Text.Split(' ')[1].Split(',')[0].Replace(".", string.Empty);
        }

        _objid = AssetCatalog.UpdateAssetHeader(_objid, _begda, _endda, txtAssetName.Text, ddlGroup.SelectedValue, ddlGroup.SelectedItem.Text, ddlSubGroup.SelectedValue, ddlSubGroup.SelectedItem.Text, txtIdNumber.Text, txtAssetNumber.Text, txtNumber.Text, ddlOrganization.SelectedValue, ddlOrganization.SelectedItem.Text, ddlLocation.SelectedValue, ddlLocation.SelectedItem.Text, ddlPerolehan.SelectedItem.Text, _tanggalPerolehan, txtPeruntukan.Text, _statusCode, _statusName, txtMTBF.Text, _nilai, txtKeterangan.Text, ddlEmployee.SelectedValue, ddlEmployee.SelectedItem.Text.Split('-')[ddlEmployee.SelectedItem.Text.Split('-').Length - 1].Trim(), (string)Session["username"]);
                            ...
                            <header class="panel-heading">
                                ...
                                <div class="panel-body">
                                    <div class="form-horizontal " runat="server">
                                        ...
                                        <div class="form-group">
                                            <label class="col-sm-2 control-label popovers" data-original-title='Informasi Aset' data-content='Mean Time Between Failures. Standard periode yang diberikan untuk melakukan perawatan terhadap aset. Silakan isi dengan nilai 0 jika asset tidak memiliki MTBF.' data-placement='right' data-trigger='hover'>MTBF (Hari)*</label>
                                            <div class="col-lg-4 col-md-4">
                                            ...
                                            <div class="col-sm-9">
                                                <asp:GridView ID="grvAttribute" Width="100%" BorderStyle="None" OnRowDeleting="OnRowDeleting" HeaderStyle-HorizontalAlign="Left" HeaderStyle-VerticalAlign="Middle" runat="server" ShowFooter="true" class="table table-striped table-bordered table-condensed" AutoGenerateColumns="false">
                                                    <Columns>
                                                        ...
                                                        <asp:CommandField ItemStyle-HorizontalAlign="Center" ItemStyle-VerticalAlign="Middle" ControlStyle-CssClass="btn btn-red" FooterStyle-BorderStyle="None"  ItemStyle-BorderStyle="None" HeaderStyle-BorderStyle="None" ButtonType="Link" ShowDeleteButton="true" ItemStyle-Width="150" DeleteText="<i class='fa fa-edit'></i>" />
                                                    </Columns>
                                                </asp:GridView>
                                            </div>
                                            <div class="col-sm-2">
