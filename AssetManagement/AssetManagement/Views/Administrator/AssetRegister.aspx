<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AssetRegister.aspx.cs" Inherits="AssetManagement.Views.Administrator.AssetRegister" %>
...
<script runat="server">
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            SetLokasi();
            SetCaraPerolehan();
            SetVisibleMaintenanceForm(false);

            SetMaintenanceGroup();
            SetMaintenanceType(ddlMaintenanceGroup.SelectedValue);
            SetPeriode();  
        }
    }
    ...
    protected void CreateDefaultFormControl()
    {
        ...
        txtPeriodeValue.Enabled = false;
        txtPeriodeValue.CssClass = "form-control form-control-inline input-medium update";
        txtPeriodeUnit.Enabled = false;
        txtPeriodeUnit.CssClass = "form-control form-control-inline input-medium update";
    }
    
    protected void SetVisibleMaintenanceForm(bool _value)
    {
        ddlMaintenanceGroup.Visible = _value;
        ddlMaintenanceType.Visible  = _value;
        txtMaintenanceLast.Visible  = _value;
        lblMaintenanceGroup.Visible = _value;
        lblMaintenanceType.Visible  = _value;    
        lblMaintenanceLast.Visible  = _value;
        lblPeriode.Visible = _value;
        txtPeriodeUnit.Visible = _value;
        txtPeriodeValue.Visible = _value;
    }        
    
    protected void cbxMaintenance_CheckedChanged(object sender, EventArgs e)
    {
        if (cbxMaintenance.Checked)
        {
            SetVisibleMaintenanceForm(true);
        }
        else
        {
            SetVisibleMaintenanceForm(false);
    ...
    protected void SetMaintenanceGroup()
    {
        DataTable data = AssetCatalog.GetTaskGroups();

        ddlMaintenanceGroup.Items.Clear();
        for (int i = 0; i < data.Rows.Count; i++)
        {
            ddlMaintenanceGroup.Items.Add(new ListItem(data.Rows[i]["TASK"].ToString(), data.Rows[i]["OBJID"].ToString()));
        }
    }

    protected void ddlMaintenanceGroup_SelectedIndexChanged(object sender, EventArgs e)
    {
        SetMaintenanceType(ddlMaintenanceGroup.SelectedValue);
        SetPeriode();
    }
    
    protected void SetMaintenanceType(string TSKID)
    {
        DataTable data = AssetCatalog.GetTaskScheduleByTaskGroup(TSKID);

        ddlMaintenanceType.Items.Clear();
        for (int i = 0; i < data.Rows.Count; i++)
        {
            ddlMaintenanceType.Items.Add(new ListItem(data.Rows[i]["STASK"].ToString(), data.Rows[i]["OBJID"].ToString()));
        }
    }
    ...
    {
        txtPeriodeValue.Text = string.Empty;
        txtPeriodeUnit.Text = string.Empty;
        
        if (ddlMaintenanceType.Items.Count > 0)
        {
            SetMaintenancePeriode(ddlMaintenanceType.SelectedValue);
        }
    }
    
    protected void SetMaintenancePeriode(string STASK)
    {
        DataTable data = AssetCatalog.GetTaskScheduleByID(STASK);
        
        if (data.Rows.Count > 0)
        {
            txtPeriodeValue.Text = data.Rows[0]["PERIO"].ToString();
            txtPeriodeUnit.Text = data.Rows[0]["PUNIT"].ToString();
        }
    }
    
    protected DateTime CalculateNextMaintenanceDate(string LastMaintenance)
    {
        DateTime _lastMaintenance = Convert.ToDateTime(DateTimeFormat.ReverseDateVersionSQL(LastMaintenance));
        int _interval = Convert.ToInt32(txtPeriodeValue.Text);
        
        if (txtPeriodeUnit.Text.ToLower() == SatuanWaktu.Tahun.ToString().ToLower())
        {
            return _lastMaintenance.AddYears(_interval);
        }
        else if (txtPeriodeUnit.Text.ToLower() == SatuanWaktu.Bulan.ToString().ToLower())
        {
    ...
    {
        DataTable data = AssetCatalog.GetAssetHeaderByAssetNumber(ASTNO);
    ...
    protected void ValidateAssetNumber(string ASTNO)
    {
        ...
        if (_objid > 0)
        {
            ...
            SetDataMaintenanceToController(_objid.ToString());
    ...
    protected void SetDataToController(string OBJID)
    {
        ...
        if (rbtCIP.Attributes["Value"] == data.Rows[0]["STACD"].ToString())
            rbtCIP.Checked = true;
        else
            rbtFinished.Checked = true;
    ...
    protected void SetDataMaintenanceToController(string PLNID)
    {
        DataTable data = AssetCatalog.GetPlantMaintenanceByPlantID(PLNID);
        
        if (data.Rows.Count > 0)
        {
            cbxMaintenance.Checked = true;
            SetVisibleMaintenanceForm(true);
            ddlMaintenanceGroup.SelectedValue = data.Rows[0]["TSKID"].ToString();
            ddlMaintenanceType.SelectedValue = data.Rows[0]["STKID"].ToString();
            txtPeriodeValue.Text = data.Rows[0]["PERIO"].ToString();
            txtPeriodeUnit.Text = data.Rows[0]["PUNIT"].ToString();
            txtMaintenanceLast.Text = data.Rows[0]["LASTM"].ToString() == string.Empty ? string.Empty : DateTimeFormat.ReverseDateVersionASP(data.Rows[0]["LASTM"].ToString());
        }
    }
    ...
    protected void InsertPlantMaintenance(string PLNID)
    {
        if (cbxMaintenance.Checked)
        {
            AssetCatalog.InsertPlantMaintenance(0, PLNID, ddlMaintenanceGroup.SelectedValue, ddlMaintenanceGroup.SelectedItem.Text, ddlMaintenanceType.SelectedValue, ddlMaintenanceType.SelectedItem.Text, txtPeriodeValue.Text, txtPeriodeUnit.Text, DateTimeFormat.ReverseDateVersionSQL(txtMaintenanceLast.Text), CalculateNextMaintenanceDate(txtMaintenanceLast.Text).ToString(), (string) Session["username"]);
        }
    }
    ...
    protected void UpdateData()
    {
        ...
        string _statusCode = rbtCIP.Checked ? rbtCIP.Attributes["Value"] : rbtFinished.Attributes["Value"];
        string _statusName = rbtCIP.Checked ? rbtCIP.Text : rbtFinished.Text;
        string _kodePlant = string.Empty;
        string _nomorAset = string.Empty;
        string _begda = DateTimeFormat.ReverseDateVersionSQL(txtBegda.Text);
        string _endda = DateTimeFormat.ReverseDateVersionSQL(txtEndda.Text);
        string _tanggalPerolehan = DateTimeFormat.ReverseDateVersionSQL(txtTanggalPerolehan.Text);
       ...
       _objid = AssetCatalog.UpdateAssetHeader(_objid, _begda, _endda, txtAssetName.Text, ddlGroup.SelectedValue, ddlGroup.SelectedItem.Text, ddlSubGroup.SelectedValue, ddlSubGroup.SelectedItem.Text, txtNumber.Text, txtAssetNumber.Text, txtIdNumber.Text, ddlOrganization.SelectedValue, ddlOrganization.SelectedItem.Text, ddlLocation.SelectedValue, ddlLocation.SelectedItem.Text, ddlPerolehan.SelectedItem.Text, _tanggalPerolehan, txtPeruntukan.Text, _statusCode, _statusName, txtMTBF.Text, txtNilaiAset.Text, txtKeterangan.Text, ddlEmployee.SelectedValue, ddlEmployee.SelectedItem.Text.Split(' ')[2], (string)Session["username"]);
        ...
        InsertPlantMaintenance(_objid.ToString());
                                    ...
                                    <div class="form-horizontal " runat="server">
                                        ...
                                        <div class="form-group">
                                            <div class="col-sm-12">
                                                ...
                                                <asp:Label runat="server" ID="Label1" ><strong>REGISTER ASET</strong></asp:Label></p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-4 col-md-4">
                                                ...
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                        <asp:TextBox ID="txtEndda" runat="server" class="form-control form-control-inline input-medium default-date-picker" ></asp:TextBox>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-4 col-md-4">
                                                ...
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                        <asp:TextBox ID="txtTanggalPerolehan" runat="server" class="form-control form-control-inline input-medium default-date-picker" ></asp:TextBox>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-4 col-md-4">
                                                <div class="col-lg-6 col-md-6">
                                                    <div class="radio">
                                                        <asp:RadioButton runat="server" GroupName="Status" ID="rbtCIP" value="0" Text="CIP" Enabled="false" />
                                                    </div>
                                                </div>
                                                <div class="col-lg-6 col-md-6">
                                                    <div class="radio">
                                                        <div class="form-group">
                                                            <div class="col-lg-2 col-md-2">
                                                                <asp:RadioButton runat="server" GroupName="Status" ID="rbtFinished" value="1" Text="Finished" Enabled="false" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-6 col-md-6">
                                                <div class="radio">
                                                    <asp:CheckBox runat="server" ID="cbxMaintenance" Text=" Include Maintenance Data" AutoPostBack="true" OnCheckedChanged="cbxMaintenance_CheckedChanged"/>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-4 col-md-4">
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                        <asp:Label runat="server" ID="lblMaintenanceGroup" class="help-block">Grup Maintenance</asp:Label>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                        <asp:DropDownList ID="ddlMaintenanceGroup" runat="server" class="chosen" AutoPostBack="true" OnSelectedIndexChanged="ddlMaintenanceGroup_SelectedIndexChanged" ></asp:DropDownList>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-lg-1 col-md-1"></div>
                                            <div class="col-lg-4 col-md-4">
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                         <asp:Label runat="server" ID="lblMaintenanceType" class="help-block">Jenis Maintenance</asp:Label>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                        <asp:DropDownList ID="ddlMaintenanceType" runat="server" class="chosen" AutoPostBack="true" OnSelectedIndexChanged="ddlMaintenanceType_SelectedIndexChanged" ></asp:DropDownList>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-4 col-md-4">
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                         <asp:Label runat="server" ID="lblPeriode" class="help-block">Periode</asp:Label>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-lg-4 col-md-4">
                                                        <asp:TextBox ID="txtPeriodeValue" runat="server" class="form-control  m-bot15"></asp:TextBox>
                                                    </div>
                                                    <div class="col-lg-8 col-md-8">
                                                        <asp:TextBox ID="txtPeriodeUnit" runat="server" class="form-control  m-bot15"></asp:TextBox>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-lg-1 col-md-1"></div>
                                            <div class="col-lg-4 col-md-4">
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                         <asp:Label runat="server" ID="lblMaintenanceLast" class="help-block">Tanggal Terakhir Maintenance</asp:Label>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                        <asp:TextBox ID="txtMaintenanceLast" runat="server" class="form-control form-control-inline input-medium default-date-picker"></asp:TextBox>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
