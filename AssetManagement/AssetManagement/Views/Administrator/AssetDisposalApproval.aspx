<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AssetDisposalApproval.aspx.cs" Inherits="AssetManagement.Views.Administrator.AssetDisposalApproval" %>
...
<script runat="server">
    ...
    protected void ValidateAssetNumber(string OBJID)
    {
        SetDataToController(OBJID);
        SetDataDisposalToController(OBJID);
        SetAssetDetailToDataTable(OBJID);
        SetImageFile(OBJID);
    }
    
    protected void SetDataDisposalToController(string OBJID)
    {
        DataTable data = AssetCatalog.GetAssetDisposalByPlantID(OBJID);

        txtNilaiBuku.Text = data.Rows[0]["WORTH"].ToString();
        txtNomorSurat.Text = data.Rows[0]["LTRNO"].ToString();
        txtDisposalDate.Text = data.Rows[0]["TRSDT"].ToString() == string.Empty ? string.Empty : DateTimeFormat.ReverseDateVersionASP(data.Rows[0]["TRSDT"].ToString());

        if (rbtNewStatusBaik.Attributes["Value"] == data.Rows[0]["STACD"].ToString())
            rbtStatusBaik.Checked = true;
        else if (rbtNewTidakProduktif.Attributes["Value"] == data.Rows[0]["STACD"].ToString())
            rbtNewTidakProduktif.Checked = true;
        else if (rbtNewStatusRusak.Attributes["Value"] == data.Rows[0]["STACD"].ToString())
            rbtNewStatusRusak.Checked = true;
        else
            rbtNewNonFisik.Checked = true;
    }
    ...
    protected void SetDataToController(string OBJID)
    {
        ...
        txtNilaiAset.Text = data.Rows[0]["WORTH"].ToString();
    ...
    protected void btnApprove_Click(object sender, EventArgs e)
    {
        ...
        {
            if (txtNilaiBuku.Text == string.Empty)
            {
            ...
            else if (txtNomorSurat.Text == string.Empty)
            {
                ClientScript.RegisterStartupScript(this.GetType(), "myalert", "alert('" + "Informasi nomor surat hapus buku aset tidak boleh kosong" + "');", true);
            }
            else
            {
                UpdateDataApprove();
                Response.Redirect("AssetDisposalApprovalList.aspx");
            }
        }
        else
        {
    ...
    protected string UploadFile(string REFNO)
    {
        if (fupLampiran.HasFile)
        {
            try
            {
                string filename = Path.GetFileName(fupLampiran.FileName);
                fupLampiran.SaveAs(Server.MapPath("../../Files/Asset_Letter/") + FileManager.RenameFile(REFNO, fupLampiran));
                return ((@"../../Files/Asset_Letter/") + FileManager.RenameFile(REFNO, fupLampiran));
            }
            finally
            {
                fupLampiran.Dispose();
    ...
    protected void UpdateDataApprove()
    {
        ...
        //DelimitAssetDetail(_plnid);
        UpdateAssetDisposal(_plnid, _refno, ((int)ApprovalStatus.Approved).ToString(), ((int)WorkflowStatus.Approve).ToString());
    }

    protected void UpdateDataInProcess()
    {
        string _plnid = Request.QueryString["PLNID"] != null ? Cryptography.Decrypt(Request.QueryString["PLNID"], true) : "0";
        string _refno = Request.QueryString["REFNO"] != null ? Cryptography.Decrypt(Request.QueryString["REFNO"], true) : "0";

        //DelimitAssetDetail(_plnid);
        UpdateAssetDisposal(_plnid, _refno, ((int)ApprovalStatus.InProcess).ToString(), ((int)WorkflowStatus.InProcess).ToString());
    }

    protected void UpdateDataDecline()
    {
        string _plnid = Request.QueryString["PLNID"] != null ? Cryptography.Decrypt(Request.QueryString["PLNID"], true) : "0";
        string _refno = Request.QueryString["REFNO"] != null ? Cryptography.Decrypt(Request.QueryString["REFNO"], true) : "0";

        //DelimitAssetDetail(_plnid);
        UpdateAssetDisposal(_plnid, _refno, ((int)ApprovalStatus.Declined).ToString(), ((int)WorkflowStatus.Decline).ToString());
    }

    protected void UpdateAssetDisposal(string PLNID, string REFNO, string approval, string flowStatus)
    {
        ...
        string _fileName = UploadFile(REFNO);
        AssetCatalog.UpdateAssetDisposal(PLNID, REFNO, approval, DateTimeFormat.ReverseDateVersionSQL(txtDisposalDate.Text), txtNilaiBuku.Text, _statusCode, _statusName, _kodePenghapusan, _jenisPenghapusan, txtNomorSurat.Text, _fileName, (string)Session["username"]);
        AssetWorkflow.InsertWorkFlow(REFNO, ((int)WorkflowUser.AssetManagementApprover).ToString(), ((int)WorkflowType.Disposal).ToString(), Session["username"].ToString(), Session["poscd"].ToString(), string.Empty, flowStatus, Session["username"].ToString());
    }
                            ...
                            <header class="panel-heading">
                                ...
                                <div class="panel-body">
                                    <div class="form-horizontal " runat="server">
                                        ...
                                        <div class="form-group">
                                        <asp:Label runat="server" ID="Label4" class="col-sm-2 control-label popovers" data-original-title='Informasi Aset' data-content='Nomor Surat Change Control (Penghapusbukuan)' data-placement='right' data-trigger='hover' Text="Nomor Surat"></asp:Label> 
                                            <div class="col-lg-4 col-md-4 col-sm-4">
                                                <asp:TextBox ID="txtNomorSurat" runat="server" class="form-control  m-bot15" Text=""></asp:TextBox>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-md-8">
                                                <div class="fileupload fileupload-new" data-provides="fileupload">
                                                    <span class="btn btn-white btn-file">
                                                        ...
                                                        <asp:FileUpload ID="fupLampiran" runat="server" class="default" />
                                                    </span>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-4 col-md-4 col-sm-4">
                                                <asp:TextBox ID="txtDisposalDate" runat="server" class="form-control form-control-inline input-medium default-date-picker"></asp:TextBox>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-lg-4 col-md-4">
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                                        <span class="help-block">Nomor Registrasi Aset Akuntansi</span>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col-lg-12 col-md-12">
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <asp:Button ID="btnInProcess" runat="server" class="btn btn-orange" Text="In Process" OnClick="btnInProcess_Click"></asp:Button>
